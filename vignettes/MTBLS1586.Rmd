...
transformations <- read_csv(transformations_file, 
                            col_names = TRUE) %>% 
  as.data.frame()
...
massdiff_df <- massdiff_df[massdiff_df$binary == 1, ]
...
```{r}
# Summary of the list
mz_sum <- mz_summary(massdiff, var = "group")
...
GNPS_score <- compareSpectra(ms2_spectra_pos,
...
colnames(GNPS_score) <- ms2_spectra_pos$id
rownames(GNPS_score) <- ms2_spectra_pos$id

spect_adj <- addSpectralSimilarity(am_structural = massdiff, 
                                   ms2_similarity = list("GNPS_score" = GNPS_score))

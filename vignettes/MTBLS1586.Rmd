...
```{r}
# Summary of the list
mz_sum <- mz_summary(massdiff)


# for big data: filter for counts e.g. higher 1000
sum_mz_f <- mz_summary(massdiff, filter = 1000)
...
anno_filter <- filter(massdiff_anno, massdiff_anno$metabolite_identification != "")
anno_filter[25:30,]
...
pears_df <- as.data.frame(pears_corr)
pears_df[25:30,]
...
ms2_similarity_df <- spec_df(ms2_similarity)
...
# note: only first 10 features are exported
exportNet2gml(x = ms2_similarity_df[1:10,], file = "similarity")
...
attributes <- merge(pears_df, massdiff_df, by = c("Row", "Col"))
...
# note: only first 10 features are exported
exportAttributes2gml(attributes[1:10,], file = "attributes_mtbls1586_pos", anno = feat_anno)
```

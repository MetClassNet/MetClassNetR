...
networks, using the `readNet` function, which gets as input the directory
...
NOTE. You can also generate a predefined set of 4 toy-networks with the `makeToyNet`
...
# read networks
networks <- readNet(netDir, directed = directed, pattern = "neg", format = "gml")
...
use the `calculNetStats` function, giving a parameter the networks you just
read with `readNet`. To generate the plots, we will use the `PrintStatsPlots`
function, which needs the results of the `calculNetStats` function as input
parameter.
